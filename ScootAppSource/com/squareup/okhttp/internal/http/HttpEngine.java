...
public final class HttpEngine
{
  ...
  private Response cacheWritingResponse(CacheRequest paramCacheRequest, Response paramResponse)
  {
    ...
    do
    {
      ...
      localac = paramCacheRequest.body();
    } while (localac == null);
    paramCacheRequest = new HttpEngine.2(this, paramResponse.body().source(), paramCacheRequest, r.a(localac));
    return paramResponse.newBuilder().body(new RealResponseBody(paramResponse.headers(), r.a(paramCacheRequest))).build();
  }
  ...
  private Response unzip(Response paramResponse)
  {
    ...
    p localp = new p(paramResponse.body().source());
    Headers localHeaders = paramResponse.headers().newBuilder().removeAll("Content-Encoding").removeAll("Content-Length").build();
    return paramResponse.newBuilder().headers(localHeaders).body(new RealResponseBody(localHeaders, r.a(localp))).build();
  }
  ...
  public i getBufferedRequestBody()
  {
    ...
    if (localObject != null)
    {
      localObject = r.a((ac)localObject);
      this.bufferedRequestBody = ((i)localObject);
      return (i)localObject;
    }
    return null;
  }
  ...
  public HttpEngine recover(IOException paramIOException, ac paramac)
  {
    if (!this.streamAllocation.recover(paramIOException, paramac)) {}
    while (!this.client.getRetryOnConnectionFailure()) {
      return null;
    }
    paramIOException = close();
    return new HttpEngine(this.client, this.userRequest, this.bufferRequestBody, this.callerWritesRequestBody, this.forWebSocket, paramIOException, (RetryableSink)paramac, this.priorResponse);
  }
...
/* Location:              D:\Android\dex2jar-2.0\classes-dex2jar.jar!\com\squareup\okhttp\internal\http\HttpEngine.class
